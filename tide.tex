...
\label{fig:r}
\end{figure}
Since the loss of large scale information in z direction will reduce the cross correlation with small l and influence the performance on large l. 
We try to recover the modes with a method called Cosmic Tidal Reconstruction. 
...
With $t_{ij}$, we calculate the variance of large scale potential 
and get the large scale density contrast.
...
(1) Following gravitational lensing procedures, decompose the symmetric, traceless tidal force tensor into 5 components, 
\begin{eqnarray}
...
$f=2\alpha(\tau)-\beta(\tau)dlnP/dlnk$ is again the coupling function, 
